...
const TravelCard = ({ image, location, name, rating, price }) => {
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden w-full max-w-sm">
      ...
      <div className="p-4">
        ...
        <h3 className="text-lg font-semibold text-[var(--color-default)] mt-2">{name}</h3>
        <p className="text-[var(--color-emphasis)] font-bold text-lg mt-1">${price}</p>
      </div>
    </div>
  );
};
